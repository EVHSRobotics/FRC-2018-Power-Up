...
import org.usfirst.frc.team2854.robot.commands.SetClamp;
...
public class DriveNearFar extends CommandGroup {
	...
	public DriveNearFar(boolean isRightSide) {
		...
		addSequential(new ClawSetpoint(-2000));//claw at sstarting angle
		addSequential(new DriveHeading(-.75, 150, 8 * multiplier ));// forward at an angle
		...
		addSequential(new Outtake(.75, .35d));//shoots cube into the switch
		addSequential(new DriveStraight(.25, 10));//drive away from switch
		addSequential(new ClawSetpoint(-1250));//claw up
