...
>>>>>>> master
/**
 * A generic drive forward command using encoders, which can use a default, parameterized, or user
 * supplied function to determine speed
 */
...
=======
   ...
   *     pass in 90
   * @param SpeedOpp supplies the function for converting normalized distance difference (0-1) to
   *     the speed(0-1)
   * @param threshHoldDistance the minimum distance from targe tto stop. Will not go past target
  ...
  public DriveDistance(
      ...
      DoubleFunction<Double> speedOpp,
      double threshHoldDistance,
      Supplier<Double> encoders) {
   ...
   * @param maxSpeed the maximum turn speed, will move at this speed if robot has traveled less than
   *     75% of the distance
  ...
  public DriveDistance(
      ...
      double minSpeed,
      double threshHoldDistance,
      Supplier<Double> gyro) {
    this(
        drive,
        target,
        (value) -> (Math.min(value, .25)) / .25d * (maxSpeed - minSpeed) + minSpeed,
        threshHoldDistance,
        gyro);
  }
>>>>>>> master
   ...
   * @param angle Implement a function or lambda that passes in the encoder value to get the current
   *     distance. The easiest way to pass in the encoder is the use the lambda expression () -> new
   *     Double(encoder.getValue()), where encoder.getValue() is your way of getting the distance
   *     from an encoder
   */
